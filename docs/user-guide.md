...
- **여러 세션**: 세션마다 **다른 방**을 담당할 수 있다(A세션=1번방, B세션=2번방).
  담당은 `--resume` 재시작 시 유지되고, 새 세션은 `/team-relay:room <방>` 으로 지정한다
  (소속 방이 하나뿐이면 자동 담당 — 대부분의 경우 신경 쓸 일이 없다).
- **담당은 실수로 뺏기지 않는다**: 세션을 켜거나 재접속하는 것만으로는 다른 세션이 담당 중인
  방을 가져오지 않는다. 그럴 땐 `[담당 실패]` 안내만 뜨고 **기존 세션이 계속 받는다**.
  굳이 이 세션으로 가져오려면 `/team-relay:room <방>` 을 직접 실행한다 — 그때만 앞 세션이
  `[수신 이전]` 알림과 함께 수신을 잃는다. (네트워크가 잠깐 끊겼다 붙는 재접속은 같은 세션이므로
  아무 알림 없이 그대로 유지된다.)
...
| **이 세션이 받을 방 지정** | `/team-relay:room <방>` 🆕 — 초대코드 불필요 (resume 하면 유지) |
...
| (방장만) 초대·추방·공지    | `/team-relay:owner invite\|kick\|notice <방> …` — 방장 지정은 관리자 |

- **세션마다 다른 방을 담당할 수 있습니다** 🆕 — A세션은 1번방, B세션은 2번방. 방마다 이름(라벨)이 달라도 신원은 하나입니다. `--resume` 으로 재시작하면 담당이 복원되고, 새 세션은 `/team-relay:room` 으로 지정합니다.
