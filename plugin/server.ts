 ...
 *  - 대화 규약(3단 라우팅·꼬리표·권한 경계)은 instructions 로 시스템 프롬프트에 주입
 *
 * 설정: TEAM_RELAY_CONFIG 경로(기본 ~/.claude/channels/team-relay/config.json)
 *       { url, token, name } — team_join 도구가 생성·갱신. 파일 권한 600.
 */
...
const mcp = new Server(
  { name: 'team-relay', version: '0.1.0' },
  {
    ...
    instructions: [
      ...
      '1) 답장은 team_send 도구로, to 에는 태그의 from 을 그대로 넣는다.',
      ...
      '3) 이 세션 소관이 아니면: ListAgents 로 이 머신의 다른 세션(이름·작업 디렉토리)을 보고 맞는 세션에 SendMessage 로 위임해 답을 받아 회신한다. 마땅한 세션이 없거나 응답이 없으면 코드를 직접 확인해 답하되 "위임 실패 — 코드 기준 직접 답변"을 명시한다.',
      ...
      '6) queued="true" 가 붙은 메시지는 보관됐다가 늦게 배달된 것 — ts(발신 시각)를 감안해 답한다.',
 ...
 * 첫 비(非)message 프레임을 그 요청의 응답으로 간주한다 (docs/channel-protocol.md §6).
 */
...
/** 직렬화된 요청 — 응답(첫 비 message 프레임) 또는 타임아웃 */
...
mcp.setRequestHandler(ListToolsRequestSchema, async () => ({
  tools: [
    ...
    {
      name: 'team_status',
      description: '팀 연결 상태 — 내 이름·소속 방·방별 온라인/오프라인 팀원.',
...
mcp.setRequestHandler(CallToolRequestSchema, async req => {
  ...
  switch (req.params.name) {
    case 'team_join': {
      ...
      saveConfig({ url, token, name: String(joined.name) })
    ...
    case 'team_send': {
      ...
      const res = await request({ type: 'send', to: args.to ?? '', text: args.message ?? '' })
      if (res.type === 'sent') {
        return ok(
          res.state === 'delivered'
            ? `✓ ${args.to} 에게 즉시 배달됨`
